...
val_loader=DataLoader(valid_set, batch_size=len(valid_set), shuffle=True, num_workers=2)
...
for epoch in range(0, num_epoch):
    ...
    if avg_train_loss < best_train_loss:
        ...
        train_path = os.path.join(model_dir, f'best_train_model_at_{epoch}_epoch_with{best_train_loss}.pt')
        torch.save(model, train_path)  # 保存最佳训练模型
    ...
    if avg_val_loss < best_val_loss:
        ...
        val_path = os.path.join(model_dir, f'best_val_model_at_{epoch}_epoch_with{best_val_loss}.pt')
        torch.save(model,val_path )  # 保存最佳验证模型
